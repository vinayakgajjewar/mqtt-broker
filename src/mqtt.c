...
static size_t unpack_mqtt_publish(const unsigned char *buf, union mqtt_header *hdr, union mqtt_packet *pkt) {
    ...
    pkt->publish = publish;
    size_t len = mqtt_decode_length(&buf);
    pkt->publish.topiclen = unpack_string16((uint8_t **) &buf, &pkt->publish.topic);
...
static size_t unpack_mqtt_subscribe(const unsigned char *buf, union mqtt_header *hdr, union mqtt_packet *pkt) {
    ...
    /*
     * The payload consists of a list of 3-tuples consisting of:
     * - Topic length
     * - Topic filter string
     * - QoS
     */
    ...
    while (remaining_bytes > 0) {
        ...
        /*
         * TODO
         * Might leak memory here if realloc() fails.
         */
...
static size_t unpack_mqtt_unsubcribe(const unsigned char *buf, union mqtt_header *hdr, union mqtt_packet *pkt) {
    ...
    /*
     * The payload consists of a list of 2-tuples containing:
     * - Topic length
     * - Topic filter string
     */
    int i = 0;
    while (remaining_bytes > 0) {
        ...
        unsubscribe.tuples[i].topic_len = unpack_string16((uint8_t **) &buf, &unsubscribe.tuples[i].topic);
...
/*
 * The below packets are literally just a header and a packet ID.
 * - PUBACK
 * - PUBREC
 * - PUBREL
 * - PUBCOMP
...
static mqtt_unpack_handler *unpack_handlers[11] = {NULL, unpack_mqtt_connect, NULL, unpack_mqtt_publish,
                                                   unpack_mqtt_ack, unpack_mqtt_ack, unpack_mqtt_ack, unpack_mqtt_ack,
                                                   unpack_mqtt_subscribe, NULL, unpack_mqtt_unsubscribe};

/*
 * Call the appropriate unpacking function depending on the header.
 */
...
/*
 * TODO
 * Returning a pointer to a static variable is not thread-safe. It's better to
 * malloc() some bytes instead.
 */
...
struct mqtt_suback *
mqtt_packet_suback(unsigned char byte, unsigned short pkt_id, unsigned char *rcs, unsigned short rcslen) {
...
struct mqtt_publish *
mqtt_packet_publish(unsigned char byte, unsigned short pkt_id, size_t topiclen, unsigned char *topic, size_t payloadlen,
...
/*
 * Release the memory allocated for a packet.
 */
...
static mqtt_pack_handler *pack_handlers[13] = {NULL, NULL, pack_mqtt_connack, pack_mqtt_publish, pack_mqtt_ack,
                                               pack_mqtt_ack, pack_mqtt_ack, pack_mqtt_ack, NULL, pack_mqtt_suback,
                                               NULL, pack_mqtt_ack, NULL};
...
/*
 * Serialize an ACK into a byte array.
 */
...
/*
 * Serialize a CONNACK into a byte array.
 */
...
/*
 * Serialize a SUBACK into a byte array.
 */
...
/*
 * Serialize a PUBLISH into a byte array for sending over the network.
 */
